...
export const DashboardCards = ({ dashboard }: DashboardCardsProps) => {
  const cards = [
    {
      helper: `${dashboard.tenant.name} · ${dashboard.tenant.plan}`,
    ...
    },
    {
      ...
      label: 'Catalog Items',
      value: `${dashboard.totals.catalogItems}`,
    },
    {
      helper: 'Total orders',
    ...
    {
      helper: `${dashboard.totals.customers} customers`,
      icon: PaidOutlinedIcon,
      label: 'Revenue',
      value: formatCurrency(dashboard.totals.revenue),
    },
  ];
              ...
              <CardContent sx={{ height: '100%' }}>
                <Stack direction="row" sx={{ height: '100%', justifyContent: 'space-between' }}>
                  <Stack spacing={1} sx={{ minWidth: 0 }}>
                    ...
                    <Typography
                      sx={{ fontSize: { sm: '1.35rem', xs: '1.2rem' }, fontWeight: 800 }}
                      variant="h5"
                    >
                      {card.value}
                    </Typography>
