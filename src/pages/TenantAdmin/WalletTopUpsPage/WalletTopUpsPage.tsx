...
import type { AdminWalletTopUpRequest, AdminWalletTopUpStatus } from '@features/admin/types/admin.types';
...
export const WalletTopUpsPage = () => {
  ...
  const columns: GridColDef<AdminWalletTopUpRequest>[] = [
    ...
    },
    {
      field: 'status',
      headerName: 'Status',
      renderCell: (params) => <Chip color={statusColors[params.value as AdminWalletTopUpStatus]} label={params.value} size="small" />,
      width: 120,
    },
    ...
    },
    {
      ...
      headerName: 'Action',
      renderCell: (params) => (
        <Button onClick={() => openReview(params.row)} size="small" sx={{ borderRadius: 999, fontWeight: 800, textTransform: 'none' }} variant="outlined">
          View
        </Button>
      ),
      sortable: false,
  ...
  return (
    <PageSection
      action={<Chip color={pendingCount ? 'warning' : 'default'} label={`${pendingCount} pending`} />}
      description="Review customer wallet top-up transfer receipts and manually approve wallet balance updates."
      title="Wallet Top-ups"
    >
      <Stack spacing={2}>
        <Stack direction={{ sm: 'row', xs: 'column' }} spacing={2} sx={{ alignItems: { sm: 'center', xs: 'stretch' }, bgcolor: 'background.paper', border: 1, borderColor: 'divider', borderRadius: 1, p: 2 }}>
          <TextField label="Status" onChange={(event) => setStatusFilter(event.target.value as StatusFilter)} select sx={{ maxWidth: { sm: 240 } }} value={statusFilter}>
        ...
        {topUpsQuery.error ? <Alert severity="error">{toApiError(topUpsQuery.error).message}</Alert> : null}
      ...
      <Dialog fullWidth maxWidth="md" onClose={closeReview} open={Boolean(reviewTarget)}>
        ...
        <DialogContent>
          {reviewTarget ? (
            <Box sx={{ display: 'grid', gap: 2, gridTemplateColumns: { md: '1fr 1fr', xs: '1fr' }, mt: 1 }}>
              <Stack spacing={1.2}>
                ...
                <Typography>
                  Status:{' '}
                  <Chip color={statusColors[reviewTarget.status]} label={reviewTarget.status} size="small" sx={{ ml: 0.5 }} />
                </Typography>
                {reviewTarget.promoCode ? <Typography>Promo code: {reviewTarget.promoCode}</Typography> : null}
                {reviewTarget.status === 'pending' ? (
                  <>
                    <TextField label="Amount to top up" onChange={(event) => setApprovedAmount(event.target.value)} required type="number" value={approvedAmount} />
                    <TextField label="Admin note" minRows={3} multiline onChange={(event) => setAdminNote(event.target.value)} value={adminNote} />
                  </>
                ) : (
                  <>
                    {reviewTarget.approvedAmount ? <Typography>Approved: {formatCurrency(reviewTarget.approvedAmount)}</Typography> : null}
                    {reviewTarget.adminNote ? <Typography>Admin note: {reviewTarget.adminNote}</Typography> : null}
                    {reviewTarget.reviewedAt ? <Typography>Reviewed: {formatDate(reviewTarget.reviewedAt)}</Typography> : null}
                  </>
                )}
              </Stack>
              <Box sx={{ border: 1, borderColor: 'divider', borderRadius: 1, overflow: 'hidden' }}>
                <Stack direction="row" spacing={1} sx={{ alignItems: 'center', bgcolor: 'grey.100', p: 1.2 }}>
                  <ReceiptLongOutlinedIcon fontSize="small" />
                  <Typography sx={{ fontWeight: 800 }}>{receiptLabel(reviewTarget)}</Typography>
                </Stack>
                {reviewTarget.receiptImageUrl ? (
                  <Box component="img" src={reviewTarget.receiptImageUrl} sx={{ display: 'block', maxHeight: 420, objectFit: 'contain', width: '100%' }} />
                ) : (
                  <Box sx={{ p: 3 }}>
                    <Alert severity="warning">Receipt preview is not available. The signed S3 URL may have expired.</Alert>
                  </Box>
                )}
              </Box>
            </Box>
        ...
        <DialogActions sx={{ justifyContent: reviewTarget?.status === 'pending' ? 'space-between' : 'flex-end', px: 3, pb: 2.5 }}>
          {reviewTarget?.status === 'pending' ? (
            <>
              <Button
                color="error"
